...
from pkg.audio.audio_processing import get_asr_model, transcribe_audio_segments, AsrConfig  # 音频处理
...
def process_video():  # 视频处理
    ...
    try:  # 尝试识别
        ...
        model = get_asr_model(asr_config)  # 获取缓存模型
...
def extract_audio_from_media(media_path: str, output_path: str, progress_callback=None) -> bool:  # 提取音频函数
    ...
    try:  # 尝试提取
        if progress_callback:  # 有进度回调
            progress_callback(25, '开始提取音频...')  # 进度回调

        # 使用FFmpeg提取音频 - 修复命令参数
        cmd = [  # FFmpeg命令
            'ffmpeg', '-i', media_path,  # 输入文件
            '-vn',  # 禁用视频
            '-acodec', 'pcm_s16le',  # 音频编解码器：16位PCM
            '-ar', '16000',  # 采样率：16kHz
            '-ac', '1',  # 声道数：单声道
        ...
        print(f"[音频提取] 执行命令: {' '.join(cmd)}")  # 调试信息

        if progress_callback:  # 有进度回调
            progress_callback(50, '正在处理音频文件...')  # 进度回调

        # 使用subprocess.run更安全地执行命令
        ...
        if progress_callback:  # 有进度回调
            progress_callback(75, '音频提取完成...')  # 进度回调

        # 检查结果
        if result.returncode == 0:  # 成功
            # 检查输出文件是否存在
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                print(f"[音频提取] 成功：文件大小 {os.path.getsize(output_path)} 字节")
                return True  # 返回成功
            else:
                error_msg = f"音频提取失败：输出文件不存在或为空 - {output_path}"
                print(f"[音频提取] {error_msg}")
                raise Exception(error_msg)
        else:  # 失败
            error_output = result.stderr  # 错误输出
            stdout_output = result.stdout  # 标准输出
            error_msg = f'FFmpeg退出码: {result.returncode}, 错误: {error_output}, 输出: {stdout_output}'
            print(f"[音频提取] 失败: {error_msg}")
            raise Exception(f'音频提取失败: {error_msg}')  # 抛出异常

    except subprocess.TimeoutExpired:  # 超时异常
        error_msg = f"音频提取超时（5分钟）: {media_path}"
        print(f"[音频提取] {error_msg}")
        return False  # 返回失败
    except Exception as e:  # 提取失败
        print(f'[音频提取] 出错: {e}')  # 打印错误
        return False  # 返回失败
