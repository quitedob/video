...
# MiniMax配置（OpenAI兼容格式，优先读取LLM_*变量）
MINIMAX_API_KEY = os.getenv('LLM_API_KEY') or os.getenv('MINIMAX_API_KEY')  # MiniMax API密钥
MINIMAX_BASE_URL = os.getenv('LLM_API_BASE_URL') or os.getenv('MINIMAX_BASE_URL', 'https://api.minimaxi.com/v1')  # MiniMax API基础URL
MINIMAX_MODEL = os.getenv('LLM_MODEL') or os.getenv('MINIMAX_MODEL', 'MiniMax-M2.5')  # MiniMax模型

OLLAMA_BASE_URL = os.getenv('OLLAMA_BASE_URL', 'http://localhost:11434/v1')  # Ollama API基础URL
OLLAMA_MODEL = os.getenv('OLLAMA_MODEL', 'gemma3:4b')  # Ollama默认模型

AI_PROVIDER = os.getenv('AI_PROVIDER', 'minimax')  # 默认AI服务提供商
...
def get_provider_config(provider=None):  # 获取AI服务提供商配置
    ...
    if provider == 'minimax':  # MiniMax配置（OpenAI兼容格式）
        if not MINIMAX_API_KEY:
            return None, 'MiniMax API密钥未配置'
        return {
    ...
    elif provider == 'deepseek':  # DeepSeek配置
...
def get_providers():  # 获取提供商列表函数
    ...
    try:  # 异常处理
        providers = {  # 提供商列表
            'minimax': {  # MiniMax
                'name': 'MiniMax',  # 名称
                'available': bool(MINIMAX_API_KEY),  # 是否可用
                'model': MINIMAX_MODEL  # 模型
            },
            'openai': {  # OpenAI
                'name': 'OpenAI',  # 名称
